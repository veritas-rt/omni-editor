...
*   **完全ローカル・1ファイル完結**: `omni-editor.html` をブラウザで開くだけ。バックエンドサーバーは不要です。
...
1.  `omni-editor.html` をダウンロードするかリポジトリをクローンします。
2.  Chrome, Edge, または File System Access API に対応したブラウザで `omni-editor.html` を開きます。（※ローカルのHTMLファイルを直接開いた場合、セキュリティ制限で一部のiframe機能が制限される場合があります。推奨はローカルサーバーの利用です）
